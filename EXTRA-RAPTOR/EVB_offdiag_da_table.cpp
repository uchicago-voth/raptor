...
EVB_OffDiag_DA_Table::EVB_OffDiag_DA_Table(LAMMPS *lmp, EVB_Engine *engine) : EVB_OffDiag(lmp,engine)
{
...
EVB_OffDiag_DA_Table::~EVB_OffDiag_DA_Table()
{
...
int EVB_OffDiag_DA_Table::checkout(int* _index)
{
...
void EVB_OffDiag_DA_Table::compute(int vflag)
{
...
void EVB_OffDiag_DA_Table::cal_g_term_sym()
{
  g_q  = 1.0;
  dg_q = 0.0;
}
...
void EVB_OffDiag_DA_Table::cal_f_term_sym()
{
  // set up tables
  Table *tb;
  int tlm1 = tablength - 1;
...
void EVB_OffDiag_DA_Table::cal_g_term_asym()
{
  error->one(FLERR,"EVB_OffDiag_DA_Table::cal_g_term_asym() not yet coded.");
}

/* ---------------------------------------------------------------------- */

void EVB_OffDiag_DA_Table::cal_f_term_asym()
{
  error->one(FLERR,"EVB_OffDiag_DA_Table::cal_f_term_asym() not yet coded.");
}

/* ---------------------------------------------------------------------- */

void EVB_OffDiag_DA_Table::cal_force_asym(int vflag)
{
  error->one(FLERR,"EVB_OffDiag_DA_Table::cal_force_asym() not yet coded.");
}

/* ---------------------------------------------------------------------- */

void EVB_OffDiag_DA_Table::init_exch_chg()
{
  if(natom>size_exch_chg) {
    size_exch_chg = natom;
    is_exch_chg = (int *) memory->srealloc(is_exch_chg,sizeof(int)*natom, "EVB_OffDiag_DA_Table:is_exch_chg");
    exch_list = (int *) memory->srealloc(exch_list,sizeof(int)*natom, "EVB_OffDiag_DA_Table:exch_list");
  }
...
void EVB_OffDiag_DA_Table::resume_chg()
{
  double *q = atom->q;
  int *molecule = atom->molecule;
  int *mol_index = evb_engine->mol_index;
...
void EVB_OffDiag_DA_Table::sci_setup(int vflag)
{
  // set up lists pointers and env variables
...
void EVB_OffDiag_DA_Table::sci_setup_mp()
{
  // set up lists pointers and env variables
...
void EVB_OffDiag_DA_Table::sci_compute(int vflag)
{
  // set up lists pointers and env variables
  map = evb_engine->molecule_map;
  natom = atom->nlocal + atom->nghost;
...
void EVB_OffDiag_DA_Table::mp_post_compute(int vflag)
{
  // init energy, virial
  if (vflag) memset(virial,0, sizeof(double)*6);
...
void EVB_OffDiag_DA_Table::null_table(Table *tb)
{
...
void EVB_OffDiag_DA_Table::free_table(Table *tb)
{
...
void EVB_OffDiag_DA_Table::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
  ...
  fgets(line,MAXLINE,fp);
  param_extract(tb,line);
  memory->create(tb->rfile,tb->ninput,"evb_offdiag_da_table:rfile");
  memory->create(tb->efile,tb->ninput,"evb_offdiag_da_table:efile");
  memory->create(tb->ffile,tb->ninput,"evb_offdiag_da_table:ffile");
...
void EVB_OffDiag_DA_Table::bcast_table(Table *tb)
{
...
void EVB_OffDiag_DA_Table::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = 0;
  tb->fpflag = 0;
...
void EVB_OffDiag_DA_Table::compute_table(Table *tb)
{
  ...
  if (tabstyle == LINEAR) {
    memory->create(tb->rsq,tablength,"evb_offdiag_da_table:rsq");
    memory->create(tb->e,  tablength,"evb_offdiag_da_table:e");
    memory->create(tb->f,  tablength,"evb_offdiag_da_table:f");
    memory->create(tb->de, tlm1,"evb_offdiag_da_table:de");
    memory->create(tb->df, tlm1,"evb_offdiag_da_table:df");
...
double EVB_OffDiag_DA_Table::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void EVB_OffDiag_DA_Table::spline_table(Table *tb)
{
  memory->create(tb->e2file,tb->ninput,"evb_offdiag_da_table:e2file");
  memory->create(tb->f2file,tb->ninput,"evb_offdiag_da_table:f2file");
