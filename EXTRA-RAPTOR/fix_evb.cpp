  ...
  Fix(lmp, narg, arg)
{
  ...
  /*** Set up additional command settings ***/
  if (narg > 6) {
    ...
    for (int i = 6;  i < narg; i++) {
      ...
      } else if(strcmp(cmd,"noreaction")==0) 
        Engine->no_reaction=true;
      ...
      } else if(strcmp(cmd,"kspace_cplx")==0) {
        if (strcmp(val,"yes") == 0) Engine->use_kspace_cplx = true;
        else if(strcmp(val,"no") == 0) Engine->use_kspace_cplx = false;
      } else if(strcmp(cmd,"partial_offload")==0) {
        if (strcmp(val,"yes") == 0) Engine->partial_gpu_offload = true;
        else if(strcmp(val,"no") == 0) Engine->partial_gpu_offload = false;
      } 
    }
  }
...
void FixEVB::setup(int vflag)
{
  ...
  // EVB snapshots and swaps neighbor->bondlist/anglelist/... around its force
  // passes (EVB_List::setup()/change_list()), which requires the topology lists
  // to be rebuilt inside Neighbor::build() at every reneighboring, BEFORE any
  // fix runs.  With a GPU pair style, FixGPU::setup() enables deferred topology
  // builds (neighbor->set_overlap_topo(1)) so that the GPU pair style calls
  // neighbor->build_topology() from inside Pair::compute() at ago == 0; that
  // deferred build would fire in the middle of EVB's list bookkeeping, clobber
  // the swapped-in ENV lists, and leave EVB re-serving stale pre-exchange
  // topology on the step after a reneighboring (garbage bonded forces).
  // FixEVB::setup() runs after FixGPU::setup(), so force the classic behavior.
  // The RAPTOR-modified GPU pair styles skip their build_topology() call when
  // overlap_topo is off.

  neighbor->set_overlap_topo(0);
...
{
  if(neighbor->ago==0) comm->forward_comm(this);
  Engine->execute(vflag);
...
void FixEVB::pack_dump()
{
    ...
    double *qeff = NULL;
    if(Engine->evb_effpair) Engine->evb_effpair->q;
    if(!qeff) qeff = atom->q;
