...
void EVB_MatrixFull::setup()
{
  ...
  for (int i = 0; i < evb_engine->ncomplex; i++) {
    if (evb_engine->all_complex[i]->nstate>max1)
      max1 = evb_engine->all_complex[i]->nstate;
    if (evb_engine->all_complex[i]->nextra_coupling>max2)
      max2 = evb_engine->all_complex[i]->nextra_coupling;
  }
  ...
  while (natom > max_atom) {
    max_atom  += 100;
    inc_atom  = true;
  }
  while (max1 > max_state) {
    max_state += 20;
    inc_state = true;
  }
  while (max2 > max_extra) {
    max_extra += 20;
    inc_extra = true;
  }
...
#define _MAX_ATOM max_atom
  ...
  if (inc_extra || inc_atom)
    memory->grow(f_extra_coupling,max_extra,_MAX_ATOM, 3,"EVB_MatrixFull:f_extra_coupling");

  if (inc_atom)
    memory->grow(f_env,_MAX_ATOM,3,"EVB_MatrixFull:f_env");
...
void EVB_MatrixFull::compute_hellmann_feynman(int vflag)
{
  ...
  for (int i = 0; i < nstate-1; i++) {
    ...
    if (evb_engine->mp_verlet && evb_engine->mp_verlet->is_master==0)
      C *= e_offdiag[i][EOFF_ARQ];
