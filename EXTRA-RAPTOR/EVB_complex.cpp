...
void EVB_Complex::search_state()
{
  ...
  while (state_head<state_tail) {
    ...
    int t;
    for (t = 0; t < state_head; t++)
      if (shell[t] < shell[state_head] && molecule_B[t]==molecule_B[state_head])
        break;
...
void EVB_Complex::refine_state(int head)
{
  ...
  while(head<nstate) {
    ...
    for (k = start; k < head; k++) {
      ...
      if (molecule_A[k] == molecule_A[head] &&
          reaction[k] == reaction[head] &&
          path[k] == path[head]) {
        delete_state(head);
        break;
      }
...
void EVB_Complex::build_cplx_map()
{
  ...
  //memset(complex_atom, 0, nall*sizeof(int));
  ...
  for (int i = 0; i < nstate; i++) {
    ...
    int j;
    for (j = 0; j < i; j++)
      if (molecule_B[j] == molecule_B[i]) break;
    if (j < i) continue;
  ...
  for (int i = 0; i < nall; i++) {
    ...
    if (complex[molecule_id]) {
      if (complex_atom[i] != id && neighbor->ago != 0) {
        //printf("complex atom %d changed: from %d to %d at t = %d on proc %d\n", i, complex_atom[i], id, update->ntimestep, comm->me);
        complex_map_changed = 1;
      }
      complex_atom[i] = id;
  ...
  for (nlocal_cplx = 0; nlocal_cplx < natom_cplx; nlocal_cplx++)
    if (cplx_list[nlocal_cplx] >= nlocal)
      break;
...
void EVB_Complex::build_state(int index)
{
  ...
  for (int i = 0; i < natom_cplx; i++) {
    ...
    if (kernel_atom[id])
      num_bond[id] = num_angle[id] = num_dihedral[id] = num_improper[id] = 0;
...
#if defined (_OPENMP)
#pragma omp parallel default(none) shared(map, nmolecule, apm)
  {
...
void EVB_Complex::update_pair_list()
{
  ...
  for (int i = 0; i < inum; i++) {
    ...
    for (int j = 0; j < numj; j++) {
      ...
      if (kernel_atom[atomi] == 0 || kernel_atom[atomj] == 0)
        continue;
      else
        jlist[j] = atomj;
    }
...
#pragma omp parallel default(none) \
  private(atomj, bool_kernel_atom_I) shared(inum, ilist, numneigh, firstneigh, kernel_atom, mol_type)
  {
