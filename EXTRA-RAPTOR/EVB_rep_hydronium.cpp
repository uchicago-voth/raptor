/* ----------------------------------------------------------------------
   EVB Package Code
   For Voth Group
   Written by Yuxing Peng, Tianying Yan
------------------------------------------------------------------------- */
...
EVB_Rep_Hydronium::EVB_Rep_Hydronium(LAMMPS *lmp, EVB_Engine *engine) : EVB_Repulsive(lmp,engine)
{
...
EVB_Rep_Hydronium::~EVB_Rep_Hydronium()
{
...
double EVB_Rep_Hydronium::switching(double denominator, double rs, double rc, double rr)
{
...
double EVB_Rep_Hydronium::dswitching(double denominator, double rs, double rc, double rr)
{
...
{ 
  ...
  double **x = atom->x;
  double **ff = evb_engine->lmp_f; // have to offset by thread, need to have it point to original lmp_f
  int *type = atom->type;
  int *molecule = atom->molecule;
  int nall = atom->nlocal+atom->nghost;
  int atp_OH = type[atom_o];
  ...
  double * vt = new double[nthreads*6]; // virial, over threads
  memset(vt,0.0,sizeof(double)*6*nthreads);
...
#pragma omp parallel default(none) shared(vt,x,ff,type,molecule,nall,atp_OH,atom_o,atom_h)\
...
#endif
 {
  ...
  // Set up pointers with thread offset
  double *v = vt + 6*tid;
  double **f = ff + nall*tid;

  double cut1 = cutoff_OO[1]*cutoff_OO[1];
  double cut2 = cutoff_HO[1]*cutoff_HO[1];
  ...
  // reduce virial across threads
  memset(virial, 0.0, sizeof(double)*6);
