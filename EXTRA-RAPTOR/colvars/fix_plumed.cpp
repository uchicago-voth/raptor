...
/* ----------------------------------------------------------------------
   Contributing authors: Gareth Tribello (Queens U, Belfast)
                         Pablo Piaggi (EPFL)
                         Da Teng (Voth Group, RAPTOR interface)
------------------------------------------------------------------------- */
  ...
  id_pe(nullptr), id_press(nullptr)
{
  ...
  if(me == 0 && screen){
    if(evb){
      fprintf(screen,"[MESSAGE] Found FixEVB Object.\n");
    } else {
      fprintf(screen,"[MESSAGE] No FixEVB Object.\n");
    }
  }
#endif // __RAPTOR
  ...
  // RAPTOR: Update the total number of atoms with ncomplex
  if(evb) natoms += evb->Engine->ncomplex;
...
#ifdef __RAPTOR
  // Update atom->nlocal with local RAPTOR cecs
...
#else
  // This check is skipped if RAPTOR is used -DT
...
// Try to find out if the domain decomposition has been updated:
  ...
  if (nlocal != atom->nlocal) {
    ...
    if(evb) {
      ...
      for(int icplx = 0; icplx < evb->Engine->ncomplex; icplx++) {
        if(evb->Engine->rc_rank[icplx] == me) {
          int tg = atom->natoms + icplx;  // tag-1 of cec
          if(gatindex[istart + indx] != tg) {
            update_gatindex = 1;
            break;
          }         
          indx++;
