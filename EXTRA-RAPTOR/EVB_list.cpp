...
#define CHANGE_LIST(t) {                                             \
    save_pairlist->inum=t##_pair.inum;                               \
...
EVB_List::EVB_List(LAMMPS *lmp, EVB_Engine *engine) : Pointers(lmp), EVB_Pointers(engine)
{
  ...
  env_pair.ilist = env_pair.numneigh = nullptr;
  evb_pair.ilist = evb_pair.numneigh = nullptr;
  cpl_pair.ilist = cpl_pair.numneigh = nullptr;
  env_pair.firstneigh = evb_pair.firstneigh = cpl_pair.firstneigh = nullptr;
...
void EVB_List::setup()
{
  ...
  if (neighbor->ago != 0 && sys_pair.inum != save_pairlist->inum) printf("sys pair changed between builds\n");
...
void EVB_List::single_split()
{
  ...
  /*******************************************************************/
  env_list_changed = 0;
  int current_env_pair_inum = env_pair.inum;
/*
  if (update->ntimestep == 61 && comm->me == 6) {
    FILE* fp = fopen("env_before.txt", "w");
    for (int i = 0; i < env_pair.inum; i++) {
      int atomi = env_pair.ilist[i];
      fprintf(fp, "i = %d: atom = %d; numj = %d; %d\n", i, atomi, env_pair.numneigh[atomi], complex_atom[atomi]);
    }
    fclose(fp);
  }
*/
  ...
  if (has_cplx_atom) {
    ...
    for (int i = 0; i < sys_pair.inum; i++) {
      ...
      if (cplx_i) {
/*
        // j-- cancels j++ (j stays at 0) while num_non_evb decreases to 0
        // this loop is actually circling around all the neighbors of atom i
...
*/
        ...
        // j-- cancels j++ and num_non_evb decreases by 1 only when the neighbor j is a complex atom
        // this loop separates the system list non-evb neighbors from complex neighbors
        // the first num_non_evb neighbors are put in the env list
        // the remaining (numj - num_non_evb) neighbors in the evb list
      ...
      // place neighbors in the evb and env lists

      if (num_non_evb == 0) {

         // all to be in the EVB pair_list, i.e. when atom i is a complex atom
        evb_pair.ilist[evb_pair.inum++] = atom_i;
        evb_pair.numneigh[atom_i] = numj;
        evb_pair.firstneigh[atom_i] = jlist;

      } else if (num_non_evb == numj) {

        // all to be in non-EVB pair_list, i.e.,  when all the neighbbors of atom i are not complex atoms
        ...
        // splitted  pair_list: num_non_evb elements are non-complex atoms

        env_pair.ilist[env_pair.inum++] = atom_i;
        env_pair.numneigh[atom_i] = num_non_evb;
        env_pair.firstneigh[atom_i] = jlist;

        // the remaining (numj - num_non_evb) neighbors are complex atoms
        // shift the firstneigh pointer by num_non_envb
  ...
  // notify that env pair list has been changed between consecutive neighbor list builds

  if (neighbor->ago != 0 && current_env_pair_inum != env_pair.inum) {
    env_list_changed = 1;
/*
    if (update->ntimestep == 61 && comm->me == 6) {
      FILE* fp = fopen("env_after.txt", "w");
      for (int i = 0; i < env_pair.inum; i++) {
        int atomi = env_pair.ilist[i];
        fprintf(fp, "i = %d: atom = %d; numj = %d; %d\n", i, atomi, env_pair.numneigh[atomi], complex_atom[atomi]);
      }
...
*/
    #ifdef EVB_LIST_DEBUG
    printf("env list changed between neigh builds: current = %d new = %d; ago = %d on proc = %d at t = %d\n",
        current_env_pair_inum, env_pair.inum, neighbor->ago, comm->me, update->ntimestep);
    #endif
    exit;
  }
