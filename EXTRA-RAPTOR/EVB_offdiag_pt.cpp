...
void EVB_OffDiag_PT::compute(int vflag)
{
  ...
  if (is_Vij_ex) {
    ...
    if (evb_kspace) {
      ...
      if (evb_kspace_cplx) evb_kspace_cplx->off_diag_energy = 0.0;
      
      // experimenting with different is_Vij_ex values rather 1 to skip kspace here
      //   1 = with kspace, 2 = debye, 3 = wolf, 4 = cgis, 5 = user-defined; 3 and 4 needs a kappa value
      if (is_Vij_ex==2) Vij_ex_short = exch_chg_debye(vflag);
        ...
        sometimes the off-diagonal exchange-charge part has the custom part that can not be directly
        ...
        */
        ...
        if (!mp_verlet) {
          ...
          // if the separate evb kspace cplx is used

          if (evb_kspace_cplx) {
            evb_kspace_cplx->A_Rq = A_Rq;
            evb_kspace_cplx->is_exch_chg = is_exch_chg;
            evb_kspace_cplx->compute_exch(vflag);

          } else {

            // otherwise fall back to the kspace with the whole grid
    ...
    if (!mp_verlet || mp_verlet->is_master==0) {
       if (evb_kspace_cplx) Vij_ex += evb_kspace_cplx->off_diag_energy;
       else if (evb_kspace) Vij_ex += evb_kspace->off_diag_energy;
    }
