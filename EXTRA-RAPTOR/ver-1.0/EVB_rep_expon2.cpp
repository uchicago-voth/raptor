...
EVB_Rep_Expon2::EVB_Rep_Expon2(LAMMPS *lmp, EVB_Engine *engine) : EVB_Repulsive(lmp,engine)
{
  int n = atom->ntypes;
...
int EVB_Rep_Expon2::data_rep(char *buf, int *offset, int start, int end)
{
  int t=start;
...
void EVB_Rep_Expon2::compute(int vflag)
{
  ...
  for(ii=1; ii<=inum; ii++) {
	...
	r2inv = 1.0 / rsq;
	r = sqrt(rsq);
        double dr = r - _r0[itype][jtype];
	aexp = _a[itype][jtype] * exp(-_b[itype][jtype] * dr * dr);
	fpair = _b[itype][jtype] * r * aexp * 2.0 * dr * r2inv;
...
void EVB_Rep_Expon2::sci_compute(int vflag)
{
...
int EVB_Rep_Expon2::checkout(int* _index)
{
  ...
  if(count>index_max) error->all(FLERR,"Error: EVB_Rep_Expon2::checkout  count>20.\n");
