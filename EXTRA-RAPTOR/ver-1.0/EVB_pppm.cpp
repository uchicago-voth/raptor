...
void EVB_PPPM::init()
{
  ...
  if (domain->triclinic)
    error->all(FLERR,"Cannot (yet) use EVB_PPPM with triclinic box");
  ...
  if(cg_buf1) memory->destroy(cg_buf1);
  if(cg_buf2) memory->destroy(cg_buf2);
  memory->create(cg_buf1, 3*cg->nbuf, "EVB_PPPM:cg_buf1");
  memory->create(cg_buf2, 3*cg->nbuf, "EVB_PPPM:cg_buf2");
...
void EVB_PPPM::pack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void EVB_PPPM::unpack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void EVB_PPPM::pack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void EVB_PPPM::unpack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
