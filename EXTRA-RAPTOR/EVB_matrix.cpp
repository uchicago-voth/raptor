...
void EVB_Matrix::save_ev_diag(int index, bool vflag, int kspace_cplx, int accumulate)
{
  double* etarget, *vtarget;

  if (index == -1) { // MATRIX_ENV
    etarget = e_env;
  ...
  if (accumulate == 0) {
    ...
    if (evb_kspace) {    
      if (kspace_cplx == 0) etarget[EDIAG_KSPACE] = force->kspace->energy;
      else etarget[EDIAG_KSPACE] = evb_kspace_cplx->energy;
    }
    ...
    // this code path is only executed when accumulating kspace part for the non-pivot states

    if (evb_kspace) {    
      if (kspace_cplx == 0) etarget[EDIAG_KSPACE] = force->kspace->energy;
      else etarget[EDIAG_KSPACE] = evb_kspace_cplx->energy;

      etarget[EDIAG_POT] += etarget[EDIAG_KSPACE];
    }
...
void EVB_Matrix::total_energy()
{
  ...
  if(evb_engine->mp_verlet) {
    ...
    if (evb_engine->mp_verlet->is_master==1) {
      n = 0;
      e_env[EDIAG_KSPACE] = ek[n];
      e_env[EDIAG_POT] += ek[n++];
    ...
    if (evb_engine->mp_verlet->rank_block==1) {
      for (int i = 0; i < evb_complex->nstate-1; i++)
        ek[n++] = e_offdiag[i][EOFF_ARQ];
      for (int i = 0; i < evb_complex->nextra_coupling; i++)
        ek[n++] = e_extra[i][EOFF_ARQ];
      ...
      n = 0;
      for (int i = 0; i < evb_complex->nstate-1; i++)
        e_offdiag[i][EOFF_ARQ] = ek[n++];
      for (int i = 0; i < evb_complex->nextra_coupling; i++)
        e_extra[i][EOFF_ARQ] = ek[n++];
    }
