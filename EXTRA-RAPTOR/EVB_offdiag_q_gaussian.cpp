...
EVB_OffDiag_Q_Gaussian::EVB_OffDiag_Q_Gaussian(LAMMPS *lmp, EVB_Engine *engine) : EVB_OffDiag(lmp,engine)
{
...
EVB_OffDiag_Q_Gaussian::~EVB_OffDiag_Q_Gaussian()
{
...
int EVB_OffDiag_Q_Gaussian::checkout(int* _index)
{
...
int EVB_OffDiag_Q_Gaussian::data_offdiag(char *buf, int* offset, int start, int end)
{
  int t = start;
...
void EVB_OffDiag_Q_Gaussian::compute(int vflag)
{
...
void EVB_OffDiag_Q_Gaussian::cal_f_term_sym()
{
  double r2_dh, r_dh;
  double r2_ah, r_ah;
...
void EVB_OffDiag_Q_Gaussian::cal_force_sym(int vflag)
{
...
void EVB_OffDiag_Q_Gaussian::init_exch_chg()
{
  if(natom>size_exch_chg) {
    size_exch_chg = natom;
    is_exch_chg = (int *) memory->srealloc(is_exch_chg,sizeof(int)*natom, "EVB_OffDiag_Q_Gaussian:is_exch_chg");
    exch_list = (int *) memory->srealloc(exch_list,sizeof(int)*natom, "EVB_OffDiag_Q_Gaussian:exch_list");
  }
...
void EVB_OffDiag_Q_Gaussian::resume_chg()
{
  double *q = atom->q;
  int *molecule = atom->molecule;
  int *mol_index = evb_engine->mol_index;
...
void EVB_OffDiag_Q_Gaussian::sci_setup(int vflag)
{
  // set up lists pointers and env variables
...
void EVB_OffDiag_Q_Gaussian::sci_setup_mp()
{
  // set up lists pointers and env variables
...
void EVB_OffDiag_Q_Gaussian::sci_compute(int vflag)
{
  // set up lists pointers and env variables
  map = evb_engine->molecule_map;
  natom = atom->nlocal + atom->nghost;
...
void EVB_OffDiag_Q_Gaussian::mp_post_compute(int vflag)
{
  // init energy, virial
  if (vflag) memset(virial,0, sizeof(double)*6);
